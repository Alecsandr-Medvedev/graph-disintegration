const options2 = ["Секунды", "Часы", "Сутки", "Годы"];
const data = [
  {
    "value": 1,
    "name": "Актиний",
...
function createForm() {
  ...
  form.classList.add("form-row", "my-3", "p-3", "contain");

  const colorDiv = document.createElement("div");
  colorDiv.classList.add("col-md-1", "mb-2");
  ...
  selectDiv.classList.add("col-md-5", "mb-2");
  const select = document.createElement("select");
  select.classList.add("form-control", "name");
  data.forEach(option => {
  ...
  inputDiv1.classList.add("col-md-4", "mb-2",);
  const input1 = document.createElement("input");
  input1.type = "number";
  input1.classList.add("form-control", "count");
  input1.placeholder = "Количество ядер";
  inputDiv1.appendChild(input1);


  const removeBtn = document.createElement("button");
  removeBtn.classList.add("btn", "btn-danger", "mb-2", "col-md-2");
...
// Обработчик события для кнопки "Добавить форму"
document.getElementById("add-form-btn").addEventListener("click", createForm);

// Создаем первую форму при загрузке страницы
createForm();
